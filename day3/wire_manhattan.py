# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 14:04:32 2019
day 3
"""
...
#make starting point 0,0
path1=[(0,0)]
path2=[(0,0)]


#get all the co-ordinates traversed by wires
def move(x,path):
    if(x[0]=='R'):      
        r=int(x[1:])
        for _ in range(r):
...
#get the entire list of points traversed            
for p in wire1:
    move(p,path1)
for p in wire2:
    move(p,path2)
...
dist=[abs(x)+abs(y) for (x,y) in common]  #as starting point is (0,0)
dist.sort()
#part1 
print(dist[1])      #as first element will be (0,0)
    

#part2
points=list(common)
mindist=[(path1.index(p) + path2.index(p)) for p in points[1:]]
mindist.sort()
print(mindist[1])
